...
JAKO_EXPORT int jakopter_connect(const char* drone_ip)
{
	...
	if (drone_ip != NULL){ // this is not a Parrot
	  //FBO Works when recvfrom is added, the socket find the route...
	  unsigned char commandBuffer[PACKET_SIZE+1];
	  socklen_t len = sizeof(addr_drone);
	  //block until the server is ready to bind...
	  recvfrom(sock_cmd,commandBuffer,PACKET_SIZE,0,(struct sockaddr*)&addr_drone,&len);
	  //LBO
	}
